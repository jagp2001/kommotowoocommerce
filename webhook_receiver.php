 ...
 * exitosa en menos de 2 segundos. Por eso este script confirma la recepción al
 * inicio y procesa WooCommerce después, evitando que errores externos desactiven
 * el webhook en Kommo.
 */
...
try {
    ...
    logMessage('Webhook recibido', ['bytes' => strlen($rawData)]);
    ...
    if ($contact === null) {
        logMessage('No se encontró un contacto utilizable con email válido; se omite WooCommerce.', [
...
/**
 * Responde inmediatamente con HTTP 200 para cumplir el SLA de 2 segundos de Kommo.
 */
...
function findCustomFieldValue($fields, string $code, array $nameHints): ?string
{
    ...
    foreach ($fields as $field) {
        ...
        $fieldCode = strtoupper((string)($field['code'] ?? ''));
        $fieldName = mb_strtolower((string)($field['name'] ?? ''), 'UTF-8');
